...
describe("Property 11: wideEvent.outcome is always exactly 'success' or 'error', never any other value or undefined", () => {
  ...
  it("withLogging: outcome is 'success' or 'error' for any 2xx status code", async () => {
    await fc.assert(
      fc.asyncProperty(status2xxArb, async (statusCode) => {
        ...
        const handler = withLogging(
          async () => new NextResponse(null, { status: statusCode }),
        );
        ...
        const wideEvent = vi.mocked(logger.info).mock.calls[0][0] as Record<
          string,
          unknown
        >;
        expect(
  ...
  it("withLogging: outcome is 'error' for any 4xx status code", async () => {
    await fc.assert(
  ...
  it("withLogging: outcome is 'error' for any 5xx status code returned by handler", async () => {
    await fc.assert(
  ...
  it("withLogging: outcome is always 'success' or 'error' for any HTTP status code", async () => {
    await fc.assert(
  ...
  it("withLogging: outcome is 'error' when handler throws an Error", async () => {
    await fc.assert(
  ...
  it("withLogging: outcome is 'error' when handler throws a non-Error value", async () => {
    await fc.assert(
      fc.asyncProperty(
