...
export default async function EventsPage(props: {
  ...
  const { wideEvent, emit } = await getRequestLogger("/fpmsevents");
  ...
  try {
    count = await getEventsCount("BAPTISE", wideEvent);
    events = await getEvents(page.toString(), eventPerPage, "BAPTISE", wideEvent);
  ...
  return (
    <main className="container my-8 min-h-[calc(100vh-10rem)]">
      ...
      <Alert
        className="mt-8 border-4 border-red-400 font-semibold"
        variant="destructive"
      >
